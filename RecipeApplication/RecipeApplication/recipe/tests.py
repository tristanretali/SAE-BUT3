...
class RESTTests(TestCase):
    ...
    def test_search_recipes(self):
        response = self.client.get('/rest/recipes/search?title=Uber')
        ...
        recipe=Recipe.objects.get(title="Uber Nachos")
        serializer_data = RecipeSerializer(recipe).data                                                                                         b  
        self.assertEqual(serializer_data, data)

    def test_search_recipes(self):
        response = self.client.get('/rest/recipes')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
